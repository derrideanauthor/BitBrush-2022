var Sprite=function(){this.active=!1;this.name='new_sprite';this.frameNum=0;this.frameData=[];this.frame;this.layerStack=[];this.layerId;this.width;this.height;this.fps=12;this.stamp=document.createElement('canvas');this.showHiddenLayers=!0}
...
Sprite.prototype.removeFrame=function(num){let frame=this.frameData.find(obj=>obj.num==num),index=this.frameData.indexOf(frame);console.log('removing frame '+num+' at index: '+index);if(index<0)return;if(this.frameData.length==1){console.log('cant delete last frame');return}
this.frameData.splice(index,1);for(let f=0;f<this.frameData.length;f++){let fdata=this.frameData[f],fnum=parseInt(fdata.num);if(fnum>num){fdata.num=fnum-1}}
...
this.setFrame(1)}
Sprite.prototype.compileGif=function(width,height,fStart,fEnd,fps,repeat,filename,quality,bgcolor){var encoder=new GIFEncoder(),bg=bgcolor||null;let newCanvas=document.createElement('canvas');newCanvas.width=width;newCanvas.height=height;let newCtx=newCanvas.getContext('2d');encoder.setQuality(quality||1);encoder.setRepeat(repeat||0);encoder.setDelay(Math.round(1000/fps));encoder.setTransparent('#666');encoder.start();for(let fnum=fStart;fnum<=fEnd;fnum++){let frameData=this.getFrameData(fnum),mixdown=frameData.mixdown;newCtx.imageSmoothingEnabled=!1;newCtx.clearRect(0,0,width,height);newCtx.drawImage(mixdown,0,0,mixdown.width,mixdown.height,0,0,width,height);encoder.addFrame(newCtx)}
encoder.finish();if(filename.length>0)encoder.download(filename+".gif");else return encoder.stream().getData()}