...
function readFile(txt){var fileData;fileData=parseFileData(txt);var newW=fileData.width,newH=fileData.height;let splashMenus=document.getElementsByClassName('splash-menu');for(let i=0;i<splashMenus.length;i++){splashMenus[i].removeAttribute('data-active')}
...
appFeedback.newMsg('File successfully imported.',2000);zoomArtboard('fit')}
function updateCanvasComponents(fileData){props.projectName=fileData.projectName;sprite.name=props.projectName;document.title='['+props.projectName+'] - '+props.appTitle;lPanel.artboard.update(sprite.width,sprite.height);cPanel.setSwatchData(fileData.presetSwatches)}
...
function createLayerThumb(layerId){let layerStack=sprite.layerProps(layerId),title=layerStack.name,visible=layerStack.visible,card=lPanel.addLayerCard(layerId,title,visible);card.addAction(['click',function(){if(props.layerId!=layerId){selectLayer(layerId)}}]);card.mapButtons({'layers-visible':toggleVisible});function toggleVisible(){toggleLayerVisibility(layerId)}}
function addNewLayer(clone){let newpos=parseInt(sprite.layer.stackPos)+1;createLayer(newpos,clone);updateDOMStack()}
function selectLayer(layerId,history){if(props.mode=='edit')destroySelection();layerElement=document.getElementById(layerId);layerContext=layerElement.getContext("2d");props.layerId=layerId;sprite.setLayer(layerId);lPanel.setActiveCard(layerId);if(lPanel.settings.isolateLayer){toggleLayerVisibility(props.layerId,!0,!0)}
...
function updateMinimap(){let map=fPanel.minimap,mw=map.width,mh=map.height;let ratioW=1,ratioH=1,pw=parseInt(sprite.width),ph=parseInt(sprite.height),px=fPanel.viewfinder.px||0,py=fPanel.viewfinder.py||0;if(pw>ph){ratioW=(pw/ph);pw=Math.round(ratioW*mh);ph=mh}
else if(pw<ph){ratioH=(ph/pw);ph=Math.round(ratioH*mw);pw=mw}
else if(pw==ph){pw=mw;ph=mh}
let view=document.getElementById("pasteboard"),w=artUIElement.width,h=artUIElement.height,vx=-(props.artX/w)*pw,vy=-(props.artY/h)*ph,vw=view.offsetWidth/w*pw,vh=(view.offsetHeight)/h*ph;let artCenterW=(view.offsetWidth/2)-props.artX,projCenterW=pw*(artCenterW/w),xMax=pw-mw;px=Math.round(mw/2-projCenterW);if(px>=0)px=0
else if(px<=-xMax)px=-xMax;let artCenterH=(view.offsetHeight/2)-props.artY,projCenterH=ph*(artCenterH/h),yMax=ph-mh;py=Math.round(mh/2-projCenterH);if(py>=0)py=0
else if(py<=-yMax)py=-yMax;fPanel.updateViewfinder(vx,vy,vw,vh,px,py,pw,ph);fPanel.updateMinimap(sprite.frame)}
function createUILayer(){let parent=document.getElementById("pasteboard"),layerData=sprite.layerStack;artUIElement=document.createElement("canvas");artUIContext=artUIElement.getContext("2d");artUIElement.id="ui-layer";artUIElement.className="layer";artUIElement.width=props.artWidth;artUIElement.height=props.artHeight;artUIElement.style.zIndex=layerData.length+3;parent.appendChild(artUIElement);hudElement=document.createElement("canvas");hudContext=hudElement.getContext("2d");hudElement.id="hud-layer";hudElement.className="layer";hudElement.width=props.artWidth;hudElement.height=props.artHeight;hudElement.style.zIndex=layerData.length+2;parent.appendChild(hudElement);artBgElement=document.createElement("canvas");artBgContext=artBgElement.getContext("2d");artBgContext.imageSmoothingEnabled=!1;artBgElement.id="ref-layer";artBgElement.className="layer";artBgElement.width=props.artWidth;artBgElement.height=props.artHeight;artBgElement.style.zIndex="0";updateBgLayer();parent.appendChild(artBgElement)}
function updateBgLayer(){let pixelSize=1*props.artScale,blockSize=pixelSize;if(isEven(props.artWidth))blockSize=4;else blockSize=5;let scale=3;if(props.bgType=='light'||props.bgType=='dark'){artBgElement.style.background=props.bg[props.bgType];artBgElement.style.backgroundSize=(blockSize*scale)+"px";artBgElement.style.backgroundRepeat='repeat'}
if(props.bgType=='color'){artBgElement.style.background=cPanel.color.getHex();props.bgType=''}
if(props.bgType=='image'){artBgElement.style.background=props.bg[props.bgType]+' #fff';artBgElement.style.backgroundSize='contain';artBgElement.style.backgroundPosition='center center';artBgElement.style.backgroundRepeat='no-repeat';console.log('new bg: '+props.bg[props.bgType])}}
function updateHUD(){let gridColor="#666",guideColor="cyan",ctx=hudContext,w=hudElement.width,h=hudElement.height,block=1*props.artScale;ctx.clearRect(0,0,w,h);ctx.imageSmoothingEnabled=!1;ctx.translate(.5,.5);if(hud.showGrid&&block>6){ctx.strokeStyle=gridColor;ctx.lineWidth=1;ctx.globalAlpha=.2;for(let i=1;i<props.artHeight;i++){let step=Math.round(i*props.artScale);ctx.beginPath();ctx.moveTo(0,step);ctx.lineTo(w,step);ctx.stroke()}
for(let i=1;i<props.artWidth;i++){let step=Math.round(i*props.artScale);ctx.beginPath();ctx.moveTo(step,0);ctx.lineTo(step,h);ctx.stroke()}}
...
function initToolbars(){initMenus();initToolSelect();initLayerTools();initFrameTools()}
function initStartupMenu(){}
function newFileProject(){this.splash.hideWindow();let w=this.ui.values.width,h=this.ui.values.height;appFeedback.muteMsg(1);props.artWidth=w;props.artHeight=h;newCanvasSprite(w,h);sprite.name=this.ui.values.name;lPanel.artboard.update(w,h);bufferElement.width=w;bufferElement.height=h;props.projectName=this.ui.values.name;document.title='['+this.ui.values.name+'] - '+props.appTitle;sprite.name=props.projectName;updateArtboard(!0);appFeedback.updateMsg('document','New Sprite Created ('+props.artWidth+'&times;'+props.artHeight+')')}
...
props.fullscreen=!1}
elem.addEventListener('webkitfullscreenchange',function(e){},!1);elem.addEventListener('mozfullscreenchange',centerArtboard,!1);elem.addEventListener('fullscreenchange',centerArtboard,!1)}
function initMenus(){fileWindow.submit=newFileProject;fileWindow.ui.updateCallback=function(val){props.projectName=val.name;sprite.name=val.name;document.title='['+val.name+'] - '+props.appTitle};fileWindow.init('full',props.lockUI);fileWindow.ui.save.addAction(saveFile);fileWindow.ui.load.addAction(openFile);function openFile(e){var file=mPanel.file,event=new MouseEvent('click',{view:window,bubbles:!0,cancelable:!0});file.dispatchEvent(event)}
...
let startupMenu=new appMenuPanel();startupMenu.submit=newFileProject;startupMenu.version=props.version;startupMenu.init('startup',props.lockUI);startupMenu.ui.load.addAction(openFile);startupMenu.splash.showWindow();startupMenu.config({width:32,height:32});props.lockUI=!0;exportMenu.init()}
function initLayerTools(){lPanel.panelMenu.mapButtons({'file-menu':openFileMenu});function openFileMenu(){fileWindow.splash.showWindow();fileWindow.config(sprite);props.lockUI=!0}
...
lPanel.artboard.ui.hud.mapButtons({'art-toggle-grid':function(){hud.showGrid=!hud.showGrid;updateHUD()},'art-toggle-guide':function(){hud.showGuides=!hud.showGuides;updateHUD()}});lPanel.setBgFunction('light',function(){props.bgType='light';updateBgLayer()});lPanel.setBgFunction('dark',function(){props.bgType='dark';updateBgLayer()});lPanel.setBgFunction('color',function(ele){props.bgType='color';updateBgLayer();ele.firstChild.style.background=cPanel.color.getHex();console.log(ele)});let fileInput=document.createElement('input');fileInput.type='file';fileInput.accept='image/x-png,image/jpeg';fileInput.addEventListener('change',function(){let file=this.files[0];reader=new FileReader();reader.onload=function(e){props.bg.image='url('+e.target.result+')';updateBgLayer()};reader.readAsDataURL(file)},!1);lPanel.setBgFunction('image',function(ele){props.bgType='image';fileInput.click()})}
...
function exportSprite(){exportMenu.splash.showWindow();exportMenu.config(sprite);props.lockUI=!0}
...
fPanel.panelMenu.button['frames-timeline'].callback=function(){let framesPanel=document.getElementById('panel-frames'),preview=document.getElementById('frames-preview')}}
...
document.addEventListener(pointerEvent.up,function(e){cursor.drawing=!1},!1);artUIElement.addEventListener(pointerEvent.up,onPointerUp,!1);function onPointerUp(e){if(pinchZoomDist==0){if(!singleClickAction(e))draw(e);cursor.pixelLastX=cursor.pixelX;cursor.pixelLastY=cursor.pixelY;cursor.mode="primary"}}
...
function singleClickAction(e){let mx=cursor.pixelX,my=cursor.pixelY,sx=cursor.pixelSX,sy=cursor.pixelSY,singleClick=!1;if(toolProps.active)destroySelection();if(toolProps.type=='brush'||toolProps.type=='fill'||toolProps.type=='eyedropper'){if(sx==mx&&sy==my){draw(e);console.log('single click');setHistory('draw');singleClick=!0}}
cursor.drawing=!1;return singleClick}
...
if(selection.active){if(keys.ctrl&&keys.keyCode==67){copySelection(!0);appFeedback.updateMsg('selection','Pixels copied to clipboard.')}
...
else{if(keys.ctrl&&keys.shift&&keys.keyCode==82){e.preventDefault();remakeSelection();appFeedback.updateMsg('selection','Selection remade.')}}
...
function selectTool(type,mode){toolProps.type=type;toolProps.mode=mode||"normal";clearUI();if(toolProps.type!='select')destroySelection();setCursor(type,mode)}
...
if(toolProps.type=="fill"){let rgba=[212,85,0,.6];buffer.clearRect(0,0,bufferElement.width,bufferElement.height);setBuffer(props.layerId);colorFill(bufferElement,mx,my,rgba,!0,!0);putBuffer(artUIContext)}}
...
function brushTool(){let size=toolProps.size,color=toolProps.pColor.getRGBA(),mx=cursor.pixelX-size+1,my=cursor.pixelY,px=cursor.pixelPrevX-size+1,py=cursor.pixelPrevY,sx=cursor.pixelSX-size+1,sy=cursor.pixelSY,lx=cursor.pixelLastX-size+1,ly=cursor.pixelLastY;showBrushSize();if(cursor.drawing){let swatch=1;if(toolProps.mode=='erase'){color='none'}
else{if(cursor.mode=="secondary"){color=toolProps.sColor.getRGBA();swatch=2}
cPanel.setRecentColor(swatch)}
if(keys.shift){let deg=findDegree(sx,sy,mx,my);console.log('shift');console.log('lx:'+lx+' ly:'+ly+' | mx:'+mx+' my:'+my);px=lx;py=ly}
...
function eyedropperTool(){let mx=cursor.pixelX-toolProps.size+1,my=cursor.pixelY,ele=document.createElement('canvas');if(cursor.drawing){let imgData;if(toolProps.mode=="normal"){let mixdown=sprite.frame.mixdown.getContext('2d');imgData=mixdown.getImageData(mx,my,1,1).data}else{setBuffer(props.layerId);imgData=buffer.getImageData(mx,my,1,1).data}
let activeColor=0,rgba=[imgData[0],imgData[1],imgData[2],round(imgData[3]/255)];if(cursor.mode=="secondary")activeColor=1;cPanel.setColor(rgba,activeColor);cursor.drawing=!1}}
...
else{console.log("user copy: to cache");ctx.drawImage(c.clip,0,0)}}else{console.log('selection : Nothing to copy!');return!1}
return!0}
...
function findDegree(x1,y1,x2,y2){let dx=x2-x1,dy=y2-y1,deg=Math.atan(dy/dx)*(180/Math.PI);return isNaN(deg)?0:deg}
