...
Component.prototype.createWindow=function(id,notClosable,callback){let splash=this.DOM;let box=document.createElement('div');box.setAttribute('data-menu',id);box.className='window';box.setAttribute('data-height','3');splash.appendChild(box);if(!notClosable)
splash.addEventListener(pointerEvent.click,function(e){if(e.target==splash){splash.removeAttribute('data-active');callback()}},!1);this.DOM=box;return this}
...
gizmo.addEventListener(pointerEvent.click,function(e){if(!thisGizmo.editing){thisGizmo.editing=!0;e.target.select()}},!1);gizmo.addEventListener('blur',function(e){if(e.target.value=='')e.target.value=thisGizmo.defaultValue;commitChanges(e);thisGizmo.editing=!1},!1);gizmo.addEventListener('keyup',function(e){commitChanges(e);if(event.which==13||event.keyCode==13){ele.blur()}},!1);function commitChanges(e){let ele=e.target,val=formatValue(ele.value);ele.value=val;updatePair();callbackFn(val)}
...
return thisGizmo}
Component.prototype.activateGizmos=function(){var sx=0,sy=0;var mousex=0,mousey=0;let app=document.getElementById('app-main');this.panel.DOM.removeEventListener(pointerEvent.down,mouseDown,!1);this.panel.DOM.removeEventListener('touchstart',mouseDown,!1);this.panel.DOM.addEventListener(pointerEvent.down,mouseDown.bind(this.panel),!1);this.panel.DOM.addEventListener('touchstart',mouseDown.bind(this.panel),!1);function mouseDown(e){this.focus=e.target.getAttribute('data-gizmo')||'none';if(this.focus!='none'){this.focusId=e.target.id;sx=getClientPos(e).x;sy=getClientPos(e).y;e.target.blur();snapValue(this.focusId);gizmoActions(e,this.focus,this.focusId)}}
document.removeEventListener(pointerEvent.up,mouseUp,!1);document.removeEventListener('touchend',mouseUp,!1);document.addEventListener(pointerEvent.up,mouseUp.bind(this.panel),!1);document.addEventListener('touchend',mouseUp.bind(this.panel),!1);function mouseUp(e){e.preventDefault();gizmoActions(e,this.focus,this.focusId);this.focus="none"}
...
return}
if(focus==='number-drag'){let element=document.getElementById(focusId),rect=element.getBoundingClientRect(),dOff=10,dx=getClientPos(e).x-(rect.left+element.offsetWidth/2),dy=getClientPos(e).y-(rect.top+element.offsetHeight/2),d=Math.sign(dy*-1)*Math.sqrt(dx*dx+dy*dy),dMax=300;if(d>=dOff||d<=-dOff){d-=dOff*Math.sign(d);let gizmo=_this.gizmo[focusId],range=gizmo.range[1]||100,append=d/dMax*range,newVal=parseInt(gizmo.snapValue+append);if(gizmo.prevValue!=newVal){gizmo.dynamicUpdate(newVal);gizmo.prevValue=newVal}}
return}
