...
function updateCanvasComponents(fileData){props.projectName=fileData.projectName;document.title='['+props.projectName+'] - '+props.appTitle;fileWindow.setName(props.projectName)}
...
function initToolbars(){initToolSelect();initLayerTools();initFrameTools();initStartupMenu()}
...
function initLayerTools(){fileWindow.newFile.action=newFileProject;fileWindow.project.setName=function(val){props.projectName=val;document.title='['+val+'] - '+props.appTitle};fileWindow.init('full');fileWindow.setName(props.projectName);fileWindow.panelMenu.mapButtons({'file-save':saveFile});fileWindow.importMenu.mapButtons({'file-open':openFile});function openFile(){mPanel.file.click()}
function saveFile(){let fileName=props.projectName.replace(/[^a-z0-9]/gi,'_').toLowerCase(),fileDataStr='$projectName:'+props.projectName+sprite.getSpriteData();var blob=new Blob([fileDataStr],{type:"text/plain;charset=utf-8"});saveAs(blob,fileName+".bitbr");appFeedback.updateMsg('document','Project saved!');project.saved=!0}
