...
function newCanvasSprite(w,h){props.artScale=round(450/props.artWidth);if(sprite.frameData.length>0)eraseCanvasSprite();sprite.init(w,h);fPanel.addThumb(sprite.frameNum,function(fnum){sprite.setFrame(fnum);updateArtboard()});fPanel.playPreview(sprite);createLayer(1);setHistory('layer');zoomArtboard('fit')}
...
if(c==']'){if(mode==3){keyValue.push(tempValue);tempValue='';continue}
if(mode==5)treeBack++;treeBack++;mode=0;continue}
if(mode==1){keyName+=c}
if(mode==2){keyValue+=c}
if(mode==3){if(c==','){keyValue.push(tempValue);tempValue=''}
else tempValue+=c}}
...
function initMainMenu(){function saveFile(){var blob=new Blob([sprite.getSpriteData(!0)],{type:"text/plain;charset=utf-8"});saveAs(blob,"pixelart.bitbr")}
